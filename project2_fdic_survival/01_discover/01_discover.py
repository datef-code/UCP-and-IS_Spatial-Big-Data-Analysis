...
回答「我手上到底有什么数据」：扫描 ``data_raw/fdic``（只读），产出文件级明细、
格式 / 目录分布、数据集分组，以及跨年文件的**列结构漂移**（生存分析要跨 32 年
追踪同一网点，列结构漂移会直接破坏主键口径）。
    ...
    report = {
        ...
        "column_drift": {
            "note": "同组文件以第一个（年份最早）文件为基准比较列集合；跨年 SOD 字段会增减，"
                    "本项目只依赖 9 个核心列，漂移不影响追踪主键 UNINUMBR",
...
def _to_markdown(report: dict, files: pd.DataFrame) -> str:
    ...
    lines = [
        ...
        "",
        f"- 扫描根：`{report['scanned_root']}`（只读）",
        f"- 扫描时间：{report['scanned_at']}",
        f"- 文件总数：**{report['file_count']}**，总体积：**{report['total_size_human']}**"
        f"（{report['total_size']:,} 字节）",

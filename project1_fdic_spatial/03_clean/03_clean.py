...
**按字段影响**（填充 / 截断置空 / 替换 / 因该字段删除的行数 / 类型转换）、
按操作影响，以及逐条可审计的决策日志。

口径全部来自 ``config/clean_plan.yaml``（改口径不改代码）；清洗算法由
``dk.clean`` 提供。输入优先复用 ② 的过程数据 ``raw_long.parquet``，
缺失时回退到只读源数据重跑。
"""
...
def run(project) -> dict:
    ...
    report = {
        ...
        "missing_cells": {
            ...
            "note": "清洗只做删除与越界置空，不做填充（见 clean_plan.forbidden）",
        },
        "impact": impact,
    ...
    project.log(f"    [③] {before_rows:,} → {after_rows:,} 行（删除 {removed:,}，{report['shape']['removed_rate']:.2%}）；"
                f"空单元格 {before_nulls:,} → {after_nulls:,}")
    return {
        "stage": STAGE,
...
def _to_markdown(report: dict, decisions: pd.DataFrame) -> str:
    ...
    lines += ["", "## 禁止动作复核", "",
              ...
              f"- 实际违反：**{len(fb['violated'])} 条**" + (f" → {fb['violated']}" if fb["violated"] else "（全部遵守）")]
    lines.append("")
    return "\n".join(lines)

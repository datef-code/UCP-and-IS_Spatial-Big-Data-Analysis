...
- H3 分辨率 `res=8`。**实测（h3 4.5.0）**：平均单元面积 **0.737328 km²**、平均边长 **0.531414 km**、
  相邻单元中心平均距离 ≈ 0.89 km（旧文档写的「0.46 km」不是 h3 4.5.0 的 R8 值，已更正）。
- `grid_disk(k=23)` 是**候选窗口**而非 10 km 环：实测最大半径 **21.7 km**、平均 **13.0 km**；
  真正的 10 km 由 haversine 距离过滤实现（候选窗口大于目标半径只会多算不会漏算，安全）。
- 标准环 `0–1 / 1–3 / 3–5 / 5–10 km`；
- **坐标精度敏感性合并环** `0–2 / 2–5 / 5–10 km`：坐标最高精度占比 < 100%
  （2023–2025 EXACT 85.98%；1994–2022 屋顶级仅 16.37%，见 `data_raw/README.md`），
  <1 km 环有系统性失真，两套都要产出。

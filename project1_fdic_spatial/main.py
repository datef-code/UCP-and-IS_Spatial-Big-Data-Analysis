# -*- coding: utf-8 -*-
"""project1_fdic_spatial —— 根编排入口（规范 ``datakit/PROJECT_STRUCTURE.md`` §4）。

按顺序执行：内置五阶段 ``01_discover`` … ``05_map``，再执行 ``datakit.yaml``
里登记的扩展阶段 ``06_estimate`` / ``07_visualize`` / ``08_conclude``。
...
用法::
    ...
    python main.py --stage 06      # 只跑扩展阶段 06_estimate
"""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="project1_fdic_spatial 流水线")

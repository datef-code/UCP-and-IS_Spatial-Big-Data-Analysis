...
window.SDP_TOOLS = {
  ...
  /* ---------------- 数据与项目：评估入口 ---------------- */
  workbench: {
    intro: '一个「评估项目」只需要两件事：导入数据 + 填写项目信息。冲击评估、风险归因、口径检验由内部一次算完，最后统一在「评估报告」交付。所有输入存在本机浏览器，不上传服务器。',
    steps: [
      { id: 'start', no: '①', name: '数据与项目', why: '导入数据（多文件 / 本机路径）+ 填写项目信息；用你自己的数据现场重估系数。', required: true },
      { id: 'intake', no: '②', name: '数据准入体检', why: '先判断能不能做、能做到什么等级——避免把不合格需求做成一份好看但没用的报告。', required: true },
      { id: 'report', no: '③', name: '评估报告', why: '结论 / 证据 / 机制 / 风险 / 口径 / 限制一次生成，可直接打印交付。', required: true },
    ],
    samples: [
      {
  ...
  /* ---------------- 变更日志 ---------------- */
  changelog: [
    { v: 'v2.5', d: '2026-09-12', items: [
      '结构收敛：产品从「六页工作台」改为「一个项目 → 一份报告」。',
      '入口合并：「工作台 + 数据接入」合并为单页「数据与项目」（导入数据 + 填写项目信息），旧地址 #workbench / #ingest 自动跳转。',
      '模块下沉：冲击评估 / 风险归因 / 口径实验室不再各占一个页面，改由内部一次算完，作为「评估报告」的 ①②③ 章。',
      '报告重做：评估报告改为自动生成的静态叙事交付物（对齐 project1 证据链 / project2 决策驾驶舱的版式），顶部门禁横幅 + KPI 条 + 结论强度 + 六章正文。',
      '按钮收敛：取消「生成 / 刷新报告」，报告进入即生成；参数滑杆全部收进报告底部的折叠「参数实验室」，并把触发事件从 input 改为 change，避免连续重算把页面拖住。',
      '旧地址 #impact / #risk / #caliber 自动落到 #report。',
    ] },

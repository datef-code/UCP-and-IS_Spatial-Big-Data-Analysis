...
可用页面：① 数据与项目（填信息 + 导入数据）· ② 数据准入体检（问卷模式）· ③ 评估报告（自动生成的交付物，
含冲击评估 / 风险归因 / 口径检验三章 + 折叠「参数实验室」）
...
```
评估项目（一个项目 → 一份报告）
 ├─ ① 数据与项目     ← 导入数据 + 填写项目信息（原「工作台」+「数据接入」合并）
 ├─ ② 准入体检       ← 门禁：能不能做、能拿到什么等级
 └─ ③ 评估报告       ← 主交付物：结论 / 证据 / 机制 / 风险 / 口径 / 限制 六章
        └─ 参数实验室（折叠）  ← 原「冲击评估 / 风险归因 / 口径实验室」的输入，改了即重算
```
...
## 七·八、v2.5：结构收敛（一个项目 → 一份报告）

v2.4 的问题是**把「过程」当成了产品**：六个平级导航页要求使用者逐个点开，
但冲击评估 / 风险归因 / 口径实验室本质上是**一次评估内部的工序**，不是四个独立产品。

v2.5 的改动：

| 动作 | 说明 |
|---|---|
| 入口合并 | 「工作台」+「数据接入」→ 单页 **「数据与项目」**（导入数据 + 填项目信息 + 载入示例） |
| 门禁保留 | **「准入体检」仍是独立一步**——Go/No-Go 必须在出报告之前发生，否则会产出一份不合格却好看的报告 |
| 模块下沉 | 冲击评估 / 风险归因 / 口径检验不再是页面，改由内部一次算完，作为「评估报告」的 ①②③ 章 |
| 报告重做 | 报告改为**自动生成的静态叙事交付物**（对齐 project1 的「证据链」与 project2 的「决策驾驶舱」版式）：门禁横幅 → KPI 条 → 结论强度与用法 → 证据/机制 → 风险归因 → 口径检验 → 限制与血缘（含报告指纹） |
| 按钮收敛 | 取消「生成 / 刷新报告」（进页面即生成）；参数滑杆全部收进报告底部**折叠的「参数实验室」**，触发事件由 `input` 改为 `change`，避免连续重算把页面拖住 |
| 旧地址兼容 | `#workbench` `#ingest` → `#start`；`#impact` `#risk` `#caliber` → `#report`；不留死链 |

**没变的（刻意保留）**：结论强度评级器是唯一来源（准入页与报告同源）、每条结果强制携带 `caveats[]` 与 `source[]`、
缺失即显示「缺失 · 已降级」不填充估算值、以及 Markdown / JSON / PDF 三种导出。
